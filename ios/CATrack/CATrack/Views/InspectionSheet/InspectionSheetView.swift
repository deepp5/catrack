...
//
//    // Only show 3 actionable states — pending is default only, not selectable
...
struct InspectionSheetView: View {
    ...
    private func generateReport() {
        ...
        Task {
            do {
                let flatChecklist: [String: String] = sections.reduce(into: [:]) { result, section in
                    for field in section.fields { result[field.label] = field.status.rawValue }
                }
    ...
    private func severityRank(_ s: FindingSeverity) -> Int {
        switch s {
        case .pending: return 0; case .pass: return 1; case .monitor: return 2; case .fail: return 3
        }
    }
    ...
    private func finalizeInspection(with report: GenerateReportResponse) {
        ...
        let record = ArchiveRecord(
            machine: machine.model, serial: machine.serial, date: Date(),
            inspector: settingsStore.inspectorName, site: machine.site, hours: machine.hours,
            riskScore: report.riskScore,
            aiSummary: summaryLines.joined(separator: "\n"),
            sections: sections, findings: allFindings, estimatedCost: 0, trends: []
        )
