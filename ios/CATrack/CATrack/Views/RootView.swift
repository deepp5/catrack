...
struct RootView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            if !hideBottomNav {
                HStack(spacing: 0) {
                    // Inspect button — navigates to picker WITHOUT clearing session
                    Button {
                        showChat = false
                        selectedTab = .newInspection
                    } label: {
...
struct InspectionPickerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                card.padding(.horizontal, 18).padding(.top, 8)
                Spacer(minLength: 0)
            }
            .padding(.top, 10)
        }
    ...
    private var card: some View {
        VStack(spacing: 12) {
            machineDropdown
            if let m = selectedMachine {
                machinePreview(m).transition(.opacity.combined(with: .move(edge: .top)))
            }
