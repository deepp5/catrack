...
struct InspectionSheetView: View {
    ...
    private func generateReport() {
        ...
        // We currently persist inspection_id in UserDefaults from the chat/start flow.
        // Keep this lightweight for hackathon MVP.
        ...
        Task {
            do {
                // Flatten the sheet into backend-friendly checklist format
...
struct SheetFieldCard: View {
    ...
    var body: some View {
            ...
            if !field.note.isEmpty || showNote {
                TextField("Add note...", text: $noteText)
                    ...
                    .onChange(of: noteText) { _ in
                        // keep live state, but only commit on submit
                    }
            }
        }
        .onTapGesture {
            // If a note exists or the user wants to add one, toggle input
        ...
        .onChange(of: field.note) { newValue in
            // If AI prefilled or external updates occur, keep the local editor synced.
